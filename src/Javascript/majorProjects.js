fetch("./data/majorProjects.json")
  ...
  .then((projects) => {
    ...
    projects.forEach((project) => {
      const html = `
        <div class="px-2 mx-4 py-2 shadow-xl rounded-md bg-white bg-opacity-90 text-teal-700 my-4 text-xs lg:text-sm 2xl:text-base leading-relaxed">
          <!-- Project Header -->
          <div id="project-head-${project.id}" class="grid grid-cols-5 px-2" style='font-family: "Inter", sans-serif;'>
            <div class="col-span-4 flex justify-start gap-4 my-2 items-center">
              ...
              <h1 class="font-semibold text-sm lg:text-base 2xl:text-xl tracking-wider">${project.subtitle}</h1>
            </div>
